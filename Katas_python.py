...
def division():
    try:
        ...
        print('La división no fue realizada.')
...
mis_mascotas = ['Mapache', 'León', 'Oso hormiguero', 'Oso', 'Golden Retriever', 'Pajarito', 'Gato', 'Tiburón', 'Avestruz']
...
def conversion_mayusc_minusc(letra):
    return letra.upper(), letra.lower()

letras = list(map(conversion_mayusc_minusc, caracteres))
print(letras)
...
lista_palabras_filtradas = list(filter(filtrar_palabras_largas, texto.split()))
...
for estudiante in calificacion_sobresaliente:
    print(f'El estudiante {estudiante["nombre"]} ha obtenido una calificación de {estudiante["calificacion"]}')
...
def palabras_anagramas(p1, p2):
    palabra_ordenada1 = sorted(p1.lower())
    palabra_ordenada2 = sorted(p2.lower()) 
...
sumatorio = lambda x, y: x + y
resultado = list(map(sumatorio, listado1, listado2)) 
print(resultado)
...
class Arbol():
    ...
    def crecer_tronco(self):  # Función para crecer la longitud del tronco
        self.tronco += 1
    
    def nueva_rama(self):  # Función para añadir una nueva rama
        self.ramas.append(1)

    def crecer_ramas(self):  # Función para hacer crecer todas las ramas
        self.ramas = [rama + 1 for rama in self.ramas]
    
    def quitar_rama(self, indice):  # Función para eliminar una rama en una posición específica
